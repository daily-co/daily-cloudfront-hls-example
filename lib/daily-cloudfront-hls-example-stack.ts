...
export class DailyCloudfrontHlsExampleStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const hlsS3Bucket = new aws_s3.Bucket(this, "hlsS3Bucket", {
    ...
    const playerPageObject = new aws_s3_deployment.BucketDeployment(
      this,
      "hlsBucketDeployment",
      {
    ...
    dailyRole.addToPolicy(
      new aws_iam.PolicyStatement({
        ...
        resources: [hlsS3Bucket.bucketArn, hlsS3Bucket.arnForObjects("*")],
      })
    );
    ...
    const signingKeyPairId = new aws_ssm.StringParameter(
      this,
      "signingKeyPairId",
      {
    ...
    const privateKeySecret = new aws_secretsmanager.Secret(
      this,
      "privateKeySecret",
      {
    ...
    lambdaRole.addToPolicy(
      new aws_iam.PolicyStatement({
        ...
        resources: [signingKeyPairId.parameterArn],
      })
    );

    const signingLambda = new aws_cloudfront.experimental.EdgeFunction(
      this,
      "signingLambda",
      {
    ...
    const corsPolicy = new aws_cloudfront.ResponseHeadersPolicy(
      this,
      "corsPolicy",
      {
    ...
    const playlistCachePolicy = new aws_cloudfront.CachePolicy(
      this,
      "playlistCachePolicy",
      {
    ...
    const segmentCachePolicy = new aws_cloudfront.CachePolicy(
      this,
      "segmentCachePolicy",
      {
    ...
    const cloudfrontDist = new aws_cloudfront.Distribution(
      this,
      "cloudfrontDist",
      {
        defaultBehavior: {
