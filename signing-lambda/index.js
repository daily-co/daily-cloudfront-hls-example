...
let privateKey = null, keyId = null;

async function init() {
  const secretResponse = await secretsmanager.getSecretValue({ SecretId: "daily-cloudfront-hls-private-key" }).promise();
  privateKey = secretResponse.SecretString;

  const paramResponse = await ssm.getParameter({ Name: "/daily-cloudfront-hls-example/key-pair-id" }).promise();
